...
from _common import (
    ...
    get_asset_kind_name, format_date_range, get_quality_score
)
...
def analyze_library(db_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    with PhotosDB(db_path) as conn:
        ...
        cursor.execute("""
            ...
            GROUP BY ZKIND
        """)
        by_kind = {get_asset_kind_name(row['ZKIND']): row['count'] 
        ...
        """)
        storage_by_kind = {
            get_asset_kind_name(row['ZKIND']): row['total_bytes'] or 0
            for row in cursor.fetchall()
        }
        ...
        """)
        top_people = [
            {'name': row['ZFULLNAME'], 'photo_count': row['photo_count']}
            for row in cursor.fetchall()
        ]
        ...
        return {
            ...
            'storage_by_kind': {
                ...
                for k, v in storage_by_kind.items()
            },
            'storage_by_year': {
                k: {'bytes': v, 'formatted': format_size(v)}
                for k, v in storage_by_year.items()
            },
