...
from _common import (
    PhotosDB, coredata_to_datetime, datetime_to_coredata, format_date_range
)
        ...
        return {
            ...
            'summary': {
                ...
                'date_range': format_date_range(
                    photos[0]['created'] if photos else None,
                    photos[-1]['created'] if photos else None
                ),
            }
        }
...
Examples:
  ...
  %(prog)s --start-date 2025-03-01 --cluster-hours 8 --narrative
    ...
    parser.add_argument('--cluster-hours', type=int, default=4,
                        help='Hours between photos to consider separate events (default: 4)')
    parser.add_argument('--narrative', action='store_true',
                        help='Output narrative text instead of JSON')
